	...
	const Ceng::UINT32 threadId, Ceng::UINT32 renderThreads)
{
	...
	for (Ceng::UINT32 k = 0; k < renderThreads; ++k)
	{
		outputs[k] = new Experimental::Task_PixelShader(batch);
	}
	...
	Ceng::Rectangle *scissorRect, Ceng::UINT32 renderThreads, Ceng::INT32 bucketYmin)
{
	...
	// align bucket counter relative to top of bucket

	Ceng::INT32 startQuadRow = (bucketYmin - yMin) / 2;

	CyclicCounter bucketId{ renderThreads, Ceng::UINT32(startQuadRow) };
	...
	for(Ceng::INT32 k=yMin;k<yMax;k+=8)
	{
		...
		for(Ceng::INT32 j=xMin;j<xMax;j+=8)
		{
			...
			if ((tileTrivialReject[0] & tileTrivialReject[1] & tileTrivialReject[2]) >= 0)
			{
				...
				CyclicCounter t{ bucketId };

				for(Ceng::UINT32 i=0;i<4;i++)
				{
					if (chainStart[i] != -1)
					{
						GenerateChain(outputs[t],chainStart[i],chainEnd[i],
			...
			if (tileMask == -1)
			{
				...
				for(Ceng::UINT32 i=0;i<4;i++)
				{
					...
					{
						GenerateChain(outputs[t],chainStart[i],chainEnd[i],chainCoverage[i],
																k+2*i);
			...
			else 
			{
				...
				for(Ceng::UINT32 i=0;i<4;i++)
				{
					...
					if (chainType[i] == FULL_CHAIN)
					{
						...
						GenerateChain(outputs[t],chainStart[i],
		...
		label_tileSkip: // if (tileMask == -1)
			...
			// Advance bucketId to next row of tiles
...
const Ceng::INT32 CR_Rasterizer::RasterizeTriangle(std::vector<Experimental::Task_PixelShader*>& outputs,
	Ceng::Rectangle* scissorRect, Ceng::UINT32 renderThreads, Ceng::INT32 bucketYmin)
{
	//******************************************************
	// Create local copies of variables
...
const CRESULT CR_Rasterizer::GenerateChain(Experimental::Task_PixelShader* batch,
	const Ceng::INT32 startQuadX, const Ceng::INT32 endQuadX,
	const Ceng::UINT32* coverageMask,
	const Ceng::INT32 chainY)
{
