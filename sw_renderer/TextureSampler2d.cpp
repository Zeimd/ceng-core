...
void TextureSampler2d::Sample2d(const Pshader::Float2& uv, Ceng::FLOAT32* out_colorVecs)
{
	...
	*/
	...
	__m128 widthVec = _mm_load1_ps((float*)&view->textures[0]->bufferWidth);
	...
	__m128 heightVec = _mm_load1_ps((float*)&view->textures[0]->bufferHeight);
...
static const Ceng::UINT32 GetMipLevelInt(const Ceng::FLOAT32 mipFactor)
{
	...
	const Ceng::FLOAT32 mipFactor, Ceng::FLOAT32* out_color)
{
	...
	// TODO: Remove branch
	if (mipLevel > view->desc.mipLevels - 1)
	{
...
// NOTE: Only works in 64-bit mode.
	...
	const Ceng::FLOAT32 mipFactor, Ceng::FLOAT32* out_color)
{
	...
	Ceng::UINT16 dimScale[2] =
	{
		Ceng::UINT16(view->textures[mipLevel]->bufferWidth),
		Ceng::UINT16(view->textures[mipLevel]->bufferHeight)
	};
	...
	addressScale.tileStepX = Ceng::INT16(view->textures[mipLevel]->tileXstep);
	addressScale.tileStepY = Ceng::INT16(view->textures[mipLevel]->tileYstep);
	addressScale.posStepX = Ceng::INT16(view->textures[mipLevel]->channels[0].unitXstep);
	addressScale.posStepY = Ceng::INT16(view->textures[mipLevel]->channels[0].unitYstep);

	AddressPair addressVec = GenerateTiledAddress(view->textures[mipLevel]->baseAddress, addressScale, uvVec);
	...
	const Ceng::FLOAT32 mipFactor, Ceng::FLOAT32* out_color)
{
	...
	{
		1,
		Ceng::UINT16(view->textures[mipLevel]->bufferWidth << 1),
		1,
		Ceng::UINT16(view->textures[mipLevel]->bufferHeight << 1)
	};
	...
	uIntVec = MinWrapCoordinate(uIntVec, view->textures[mipLevel]->bufferWidth);
	vIntVec = MinWrapCoordinate(vIntVec, view->textures[mipLevel]->bufferHeight);
	...
	AddressPair topLeftAdr = GenerateTiledAddress(view->textures[mipLevel]->baseAddress, addressScale, uvVec);
	...
	*/
	...
	steppedU = MaxWrapCoordinate(steppedU, view->textures[mipLevel]->bufferWidth);

	__m128i uvTemp = _mm_slli_epi32(vIntVec, 16);

	uvTemp = _mm_or_si128(uvTemp, steppedU);

	AddressPair topRightAdr = GenerateTiledAddress(view->textures[mipLevel]->baseAddress, addressScale, uvTemp);
	...
	*/
	...
	steppedV = MaxWrapCoordinate(steppedV, view->textures[mipLevel]->bufferHeight);

	uvTemp = _mm_slli_epi32(steppedV, 16);

	uvTemp = _mm_or_si128(uvTemp, uIntVec);

	AddressPair bottomLeftAdr = GenerateTiledAddress(view->textures[mipLevel]->baseAddress, addressScale, uvTemp);
	...
	const Ceng::FLOAT32 mipFactor, Ceng::FLOAT32* out_color)
{
	...
	for (k = 0; k < 4; ++k)
	{
		...
		uInt[k] = uInt[k] * (view->textures[mipLevel]->bufferWidth);

		uInt[k] = uInt[k] >> 16;

		vInt[k] = vInt[k] * (view->textures[mipLevel]->bufferHeight);
	...
	for (Ceng::UINT32 k = 0; k < 4; ++k)
	{
		...
		*/
		...
		texelPtr = (Ceng::UINT8*)view->textures[mipLevel]->baseAddress;

		texelPtr += view->textures[mipLevel]->tileXstep * tileX;
		texelPtr += view->textures[mipLevel]->tileYstep * tileY;

		texelPtr += view->textures[mipLevel]->channels[0].unitYstep * posY;
		texelPtr += view->textures[mipLevel]->channels[0].unitXstep * posX;
	...
	*/
	...
	{
		texelPtr = (Ceng::UINT8*)
			(view->textures[mipLevel]->baseAddress +
				view->textures[mipLevel]->bytesPerPixel * uInt[k] +
				view->textures[mipLevel]->tileYstep * vInt[k]);
	...
	Ceng::UINT32 mipLevel, const Ceng::FLOAT32 mipFactor, Ceng::FLOAT32* out_color)
{
	...
	for (k = 0; k < 4; ++k)
	{
		...
		uInt[k] = uInt[k] * (view->textures[mipLevel]->bufferWidth) - 32768;
		...
		if (uInt[k] < 0)
		{
			uInt[k] += view->textures[mipLevel]->bufferWidth;
		}

		vInt[k] = vInt[k] * (view->textures[mipLevel]->bufferHeight) - 32768;
		...
		if (vInt[k] < 0)
		{
			vInt[k] += view->textures[mipLevel]->bufferHeight;
		}
	...
	for (Ceng::UINT32 k = 0; k < 4; ++k)
	{
		...
		// Wrap mode
		if (u == view->textures[mipLevel]->bufferWidth)
		{
			u = 0;
		}
		...
		// Wrap mode
		if (v == view->textures[mipLevel]->bufferHeight)
		{
			v = 0;
		}
